...
$worddata = mysql_query($sqlgetword);
$wordinfo = mysql_fetch_array($worddata);
...
$stemdata = mysql_query($sqlgetstems);
$stemtotal = mysql_num_rows($stemdata);
...
$sqlgetinstances = "SELECT Title, Paragraphs.WorkID, COUNT(*) AS InstanceCount
					...
					GROUP BY Title";
$instanceinfo = mysql_query($sqlgetinstances);
$totalworks = mysql_num_rows($instanceinfo);
...
$sqlgettotal = "SELECT COUNT(*) AS InstanceCount
					FROM Paragraphs
					WHERE PlainText REGEXP \"[^a-z\'\-]" . $wordplain . "[^a-z\'\-]\"";
$totalinstancedata = mysql_query($sqlgettotal);
$totalinstanceinfo = mysql_fetch_array($totalinstancedata);
	...
	<tr>
		<td>
			...
			if ($stemtotal > 0) {
				...
				$stemcount = 1;
				while ($currentstem = mysql_fetch_array($stemdata)) {
...
<table align="center">
	<tr>
		<td>
			<p>
			<?php
			while ($currentinstance = mysql_fetch_array($instanceinfo)) {
