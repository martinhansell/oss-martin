...
$queryexe = @mysql_query($sqlquery);
$currentrow = mysql_fetch_array($queryexe);
...
$sqlquery = "SELECT DISTINCT Section
				 ...
				 WHERE WorkID='$playid' 
				 ORDER BY Section";
$actquery = @mysql_query($sqlquery);

print("<h2 style='font-size:17px;'><strong>Scenes<br><span style='font-size:10px'>(" . mysql_numrows($queryexe) . " total)</span></strong></h2>\n");

while($currentrow = mysql_fetch_array($actquery)) {
	...
	$sqlquery = "SELECT Chapter, Description, ChapterID
					...
					WHERE WorkID='$playid' 
						AND Section=$act 
					ORDER BY Chapter";
	$scenequery = @mysql_query($sqlquery);
	
	// get the first scene, so we know where to start rendering the entire act
	$currentscene = mysql_fetch_array($scenequery);
	...
	while($currentscene = mysql_fetch_array($scenequery)) {		
		$scene = $currentscene["Chapter"];
		$setting = $currentscene["Description"];
...
//show each character in the play
print("<h2 style='font-size:17px;'><strong>Characters<br><span style='font-size:10px'>(" . mysql_numrows($queryexe) . " total)</span></strong></h2>\n<ul>");

while($currentrow = mysql_fetch_array($queryexe)) {
