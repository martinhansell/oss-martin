...
$sceneinfo = mysql_fetch_array($scenequery);
...
$linequery = mysql_query($sqlquery);
...
$throwaway = mysql_fetch_array($linequery);
if ($scene == 1) {
   $throwaway = mysql_fetch_array($linequery);
}

while($currentline = mysql_fetch_array($linequery)) {
	...
	$sqlquery = "SELECT CharName
				 	 FROM Characters
				 	 WHERE CharID='$charid'";
	$charquery = mysql_query($sqlquery);
	$charget = mysql_fetch_array($charquery);
