...
$workquery = "SELECT WorkID, Title 
              FROM Works 
			  ORDER BY Title";
$workdata = mysql_query($workquery);

// Play dates (list of publication years)
$datequery = "SELECT Date 
			  FROM Works 
			  ORDER BY Date";
$datedata = mysql_query($datequery);

// Play max/min dates
$dateminmaxquery = "SELECT MAX(Date) AS MaxDate, 
                           MIN(Date) AS MinDate 
			  		FROM Works";
$dateminmaxdata = mysql_query($dateminmaxquery);
$dateminmaxdataget = mysql_fetch_array($dateminmaxdata); // get the high and low dates
...
$charquery = "SELECT CharID, CharName
			  FROM Characters 
			  ORDER BY CharName";
$chardata = mysql_query($charquery); 

// Genres
$genrequery = "SELECT GenreType, GenreName
			   FROM Genres
			   ORDER BY GenreName";
$genredata = mysql_query($genrequery); 
    ...
    <tr align="left" valign="top"> 
      ...
      <td> <select name="works[]" size="10" multiple id="select10">
          ...
          <?php 
						   ...
						   while ($currentrow = mysql_fetch_array($workdata)) {
      ...
      <td> <select name="characters[]" size="10" multiple id="select11">
          ...
          <?php 
						   ...
						   while ($currentrow = mysql_fetch_array($chardata)) {
    ...
    <tr align="left" valign="top"> 
      ...
      <td rowspan="2"><select name="genres[]" size="6" multiple>
          ...
          <?php 
						   ...
						   while ($currentrow = mysql_fetch_array($genredata)) {
      ...
      <td> <p> 
          <select name="lowdate" id="select14">
            <?php 
						   ...
						   while ($currentrow = mysql_fetch_array($datedata)) {
